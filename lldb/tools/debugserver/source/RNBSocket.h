...
public:
  ...
  rnb_err_t useFD(int read, int write);
  ...
  rnb_err_t Write(const void *buffer, size_t length);
    ...
    bool IsConnected() const { return m_fd_out != -1 && m_fd_in != -1; }
...
protected:
  ...
  int m_fd_out, m_fd_in; // Socket we use to communicate once conn established
